...
@section('content')
    ...
    <div class="main-content">
        <div class="main-content-inner">
            ...
            <div class="page-content">
                <form action="{{route('miscellaneous.collect_fee')}}" method="post">
                    {{ csrf_field() }}
                    <div class="row">
                        <div class="col-sm-12">
                            @if(Session::has('msG'))
                            ...
                            <table class="table table-striped">
                                ...
                                <tr>
                                    ...
                                    @else
                                        ...
                                        <td>
                                            {{ Form::select('course', $course_list, '', ['class'=>'cors_misc form-control selectpicker', 'required'=>'required', 'data-live-search'=>'true'])}}
                                            <span class="error">{{ $errors->first('course') }}</span>
                                        </td>
                                    @endif
                                    <td style="padding-top:10px;">
                                    ...
                                    <td>
                                        {{ Form::select('student', $student_list, '', ['class'=>'student_misc form-control selectpicker', 'data-live-search'=>'true','id'=>'std'])}}
                                    </td>
                                    <!-- <td><b>OR</b></td> -->
    ...
    <script>
        ...
        $('#search_no').on('click',function(){
           ...
           var month=$('#due_month').val();
            $.post('{{route('miscellaneous.get_std_id')}}',{reg_no:reg_no,_token:'{{csrf_token()}}',session:session,branch:brnch},function (response){
                ...
                if(data.error){
                    ...
                    if((cors_id!="" && cors_id!=null) && (stud!="" && stud!=null)){
                         $.post("{{route('miscellaneous.student_fee')}}", {branch:brnch, ssn:session, course:cors_id, student:stud,due_month:month, _token:'{{ csrf_token() }}'}, function(response){
                
                        $('.fee_box').html(response);
                    });
                    $.post("{{route('miscellaneous.student_fee_history')}}", {branch:brnch, ssn:session, course:cors_id, student:stud, _token:'{{ csrf_token() }}'}, function(response){
       ...
       $('.student_misc').change(function(){
            var cors_id=$('select.cors_misc').val(); 
            var stud = $(this).val(); 
            var brnch=$('.branch_drop').val();
            var month=$('#due_month').val();
            var session=$('.sesn').val();
       ...
       $('.cors_misc').change(function(){
            ...
            $.post("{{route('student_select')}}", {branch:brnch,selected_session:selected_session,semester:smstr,course:cors_id, _token:'{{ csrf_token() }}'}, function(response){
                $('select.student_misc').html(response);
