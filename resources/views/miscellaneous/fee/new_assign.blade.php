    ...
    <div class="main-content">
        <div class="main-content-inner">
            ...
            <div class="page-content">
                ...
                <form action="{{route('miscellaneous.new_assign_fee')}}" method="post">
                    {{ csrf_field() }}
                            ...
                            @endif
                                ...
                                @foreach($head_list as $heads)
                                    <tr style="display:none;" class="dspl_{{$heads->id}}">
                                        ...
                                        <td>Due Month</td>
                                        <td>{!!Form::select('due_month[]',$months_list,null,['class'=>'form-control ttla'])!!}</td>
                                    </tr>               
                                    @foreach($sub_heads as $key =>$val)
                                        @if($heads->id==$key && (count($val)>0))
                                            @foreach($val as $k=>$v)
                                                <tr style="display:none;" class="dspl_{{$v->id}}" id="123">
                                                    <td> {{$v->fee_head_title}}</td>
                                                    <td id="aaa">
                                                        <input type="number" name="fee_amnt[]" placeholder="Enter {{$v->fee_head_title}} Amount" class="ttla form-control" />
                                                        <input type="hidden" name="fee_hd[]" value="{{$v->id}}" class="form-control" />
                                                    </td>
                                                    <td>Due Month</td>
                                                    <td>{!!Form::select('due_month[]',$months_list,null,['class'=>'form-control ttla'])!!}</td>
                                                </tr>   
                            ...
                            @foreach($assign_list as $x)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="{{route('miscellaneous.assignFee.delete',$x->id)}}" class="btn btn-minier btn-danger bootbox-confirm"><i class="fa fa-trash"></i></a>
                                        </td>
                                    </tr>
                            @endforeach
            ...
            <div class="modal-content" style="padding: 10px;">
                ...
                <div class="modal-body" >
                    ...
                    <div class="row" id="modal_row">
                        ...
                        {!!Form::open(['route'=>'miscellaneous.edit.assignfee','method'=>'POST','class'=>'form-horizontal'])!!}
                        @if(Session::get('isCourseBatch'))
...
<script type="text/javascript">
   function showSubmenu($id){
    ...
    function editassign($id){
        ...
        $
       var assign_id=$id;
       $.post('{{route('miscellaneous.loadAssignFees')}}',
       {
        id:assign_id,
        _token:'{{csrf_token()}}'
       },
