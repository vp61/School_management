...
<input type="hidden" name="edt" value="{{ $db['id'] or old('edt')}}">
    @foreach($flds as $fld=>$lbl)
        ...
        <div class="row form-group">
            ...
            <div class="col-sm-8">
                ...
                {{$errors->first($fld)}}
                @else
                    @if($fld=='is_course_batch')
                         {{Form::select($fld,[''=>'--Select--','1'=>'YES','0'=>'NO'], $default, ['class'=>'form-control'])}}
                    @else
    ...
    @endforeach
     <div class="row form-group">
        @php $default = (isset($db['sea_type'])) ? $db['sea_type']:""; @endphp
                {!! Form::label('Sea-type','Sea Type', ['class' => 'col-sm-4 control-label']) !!}
            <div class="col-sm-8">
                {{Form::select('sea_type',[''=>'--Select--','1'=>'Pre-Sea','2'=>'Post-Sea','1,2'=>'Both'], $default, ['class'=>'form-control'])}}
                <span class="error">{{$errors->first($fld)}}</span>
            </div>
        </div>
...
<a class="green" href="{{route('branch_edit',[$tbl->id])}}">
