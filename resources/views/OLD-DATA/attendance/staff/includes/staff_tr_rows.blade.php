...
@foreach($exist as $staff)
    <tr class="option_value" style="background:lightgrey">
        ...
        <td>
            <input type="hidden" name="staffs_id[]" value="{{ $staff->staff_id }}">
            {{ $staff->reg_no }}
        </td>
        <td>
        ...
        <td>
            @foreach($attendanceStatus as $status)
                <label class="pos-rel">
                    {!! Form::radio($staff->staff_id, $status->id, ($staff->$day==$status->id?true:false), ['class' => 'ace',"required"]) !!}
