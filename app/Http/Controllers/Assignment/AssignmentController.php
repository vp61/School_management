        ...
        // }
         ...
         if(isset(auth()->user()->hook_id)){
                $subjects= DB::table('timetable_subjects')
                  ...
                  ->where('timetable_subjects.course_id',$request->faculty_id)
                  ->where('timetable_subjects.section_id',$request->semester_id)
                  ->where('timetable_subjects.session_id',$request->session_id)
                  ->where('timetable_subjects.status',1)
                  ->where('tas.status',1)
                  ->where('tas.staff_id',auth()->user()->hook_id)
                  ->get();
