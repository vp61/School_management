...
use View, AppHelper, Image, URL,DB,Session;
    ...
    public function changePassword(Request $request){
        ...
        if($request->all())
        {
           ...
           $this->validate($request,$rules,$msg);
            // dd(bcrypt($request->current_password),$data['row']->password);
           ...
           if(Hash::check($request->current_password, Auth::user()->password)){
            ...
            return redirect()->back()->with('message_warning','Current password not matched');
           }
        }    
        return view(parent::loadDataToView($this->view_path.'.change-password.index'));
    }
     public function changePasswordAll(Request $request){
        $student = DB::table('users')->select('users.id as id'
            ,'std.first_name as student_name','std.reg_no','users.email as login_email','fac.faculty as course','sem.semester')
        ->leftjoin('student_detail_sessionwise as sts','sts.student_id','=','users.hook_id')
        ->leftjoin('students as std','std.id','=','sts.student_id')
        ...
        foreach ($student as $key => $value) {
            $new_pass = rand(4444,999999);
            DB::table('users')->where('id',$value->id)->update([
                'password' => bcrypt($new_pass)
            ]);
            $data[$value->id]['course'] = $value->course;
            $data[$value->id]['sec'] = $value->semester;
            $data[$value->id]['name'] = $value->student_name;
            $data[$value->id]['reg_no'] = $value->reg_no;
            $data[$value->id]['login_email'] = $value->login_email;
            $data[$value->id]['pass'] = $new_pass;
        }
        return view(parent::loadDataToView($this->view_path.'.change_password'),compact('data'));
    }
}