...
/*--teacher access--*/
use App\Models\TeacherCoordinator,DB,Auth;
/*--teacher access--*/
...
for($i=1; $i<31; $i++){
    $day = 'day_'.$i;
         ...
         $data['p'] = Attendance::select('attendances.attendees_type', 'attendances.link_id',
                'attendances.years_id', 'attendances.months_id')
                ->where('attendances.attendees_type', 1)
                ->where('attendances.'.$day , 1)
                ->where(function ($query) use ($request) {
                    if ($request->has('year') && $request->get('year') != 0) {
                        $query->where('attendances.years_id', '=', $request->year);
                        $this->filter_query['attendances.years_id'] = $request->year;
        ...
        dd($data['p']);
        // $data['a'] = Attendance::where('attendees_type',1)->where('day_'.$day,2)->where('months_id',$mm)-> leftjoin('student_detail_session
        //     wise','student_detail_sessionwise.student_id','=','attendances.link_id')->where('session_id', Session::get('activeSession'))
        // ->count();
...
, ['students.branch_id', '=' , session('activeBranch')]];
        $attendanceExist = Attendance::select('attendances.attendees_type','attendances.link_id',
            ...
            ->where('attendances.months_id',$month)
            ->where($day,'<>',0)
            ->where([['sts.course_id', '=' , $faculty], ['sts.Semester', '=' , $semester]])
            ->leftjoin('students as s','s.id','=','attendances.link_id')
            ->leftjoin('student_detail_sessionwise as sts',function($j){
                    $j->on( 's.id', '=', 'sts.student_id')
                    ->where('sts.session_id','=',Session::get('activeSession'));
