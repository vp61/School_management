...
class HomeController extends CollegeBaseController
{
    ...
    {
        $this->panel = env("course_label").' | Staff User';
        $id = auth()->user()->hook_id;
        $data = [];
   ...
   ->leftjoin('timetable_subjects as sub','sub.id','=','tb.timetable_subject_id')
    ...
    public function live_class(Request $request)
    {
      ...
      $data['meeting'] =  LiveClass::select('live_classes.*','fac.faculty as faculty','sem.semester as section')
      ->leftjoin('faculties as fac','fac.id','=','live_classes.faculty_id')
      ...
      ->get();      
      return view(parent::loadDataToView($this->view_path.'.live_class.create'),compact('api_key','secret_key','email','data'));
    }
    public function host_class($meeting_id,$meeting_topic){
      ...
      return view(parent::loadDataToView($this->view_path.'.live_class.host'),compact('row'));
    }
}
