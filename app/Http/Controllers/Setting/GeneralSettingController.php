...
class GeneralSettingController extends CollegeBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $data['row'] = GeneralSetting::select('id','created_by', 'last_updated_by', 'institute', 'salogan', 'address',
            ...
            'instagram', 'whatsApp', 'skype', 'pinterest', 'status','online_admission_session','receipt_footer','online_admission_min_pay','live_class_scheduling')->first();

        $data['url'] = '';
        $data['session']=Session_Model::select('id','session_name')->where('status',1)->pluck('session_name','id')->toArray();
        $data['session']=array_prepend($data['session'],"--Select Online Admission Session--",'');
