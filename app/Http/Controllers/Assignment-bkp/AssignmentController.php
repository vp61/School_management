...
class AssignmentController extends CollegeBaseController
{
    ...
    public function add(Request $request)
    {
        $data = [];
        $data['faculties'] = [];
        $data['semesters'] = $this->getClassTeacherSection();
        $data['subjects'] = [];
    ...
    public function edit(Request $request, $id)
    {
        ...
        $data['semesters'] = [];
        $data['semesters'][0] = 'Select Semesters';
        foreach (Semester::select('id', 'semester')->orderBy('semester', 'Asc')->get() as $smstr) {
            $data['semesters'][$smstr->id] = $smstr->semester;
        }

        $data['subjects'] = [];
        $data['subjects'][0] = 'Select Subjects';
        foreach (Subject::select('id', 'title')->orderBy('title', 'Asc')->get() as $subj) {
            $data['subjects'][$subj->id] = $subj->title;
        }
        $data['url'] = URL::current();
