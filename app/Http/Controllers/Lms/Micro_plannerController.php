...
class Micro_plannerController extends CollegeBaseController
{
  
  protected $base_route = 'Lms.micro_planner';
    protected $view_path = 'Lms.micro_planner';
    protected $panel = 'micro planner';
    protected $folder_path;
    protected $folder_name = 'micro_planner';
  ...
  public function index(Request $request){
        ...
        $data['Micro_planner']=Micro_planner::select('micro_planners.id','micro_planners.months_id','micro_planners.faculty','micro_planners.semesters_id','subjects_id','chapter_no_id','faculties.faculty','semesters.semester','sm.title as subject','chapter_no.title','micro_planners.publish_date',DB::raw("CONCAT(staff.first_name,' ',staff.last_name) as created_by"),'micro_planners.end_date','micro_planners.status as st','micro_planners.file','micro_planners.topic')
        ->leftjoin('faculties','micro_planners.faculty','=','faculties.id')
        ->leftjoin('semesters','micro_planners.semesters_id','=','semesters.id')
        ->leftjoin('timetable_subjects','micro_planners.subjects_id','=','timetable_subjects.id')
        ->leftjoin('subject_master as sm','sm.id','=','timetable_subjects.subject_master_id')
        ->leftjoin('chapter_no','micro_planners.chapter_no_id','=','chapter_no.id')
        ->leftjoin('staff','staff.id','=','micro_planners.created_by')
        ->where('micro_planners.branch_id', session('activeBranch'))
        ->where('micro_planners.session_id', session('activeSession'))
        ->where(function($query)use($request){

            if($request->faculty){
              $query->where('micro_planners.faculty',$request->faculty);
            }
            if($request->semesters_id){
              $query->where('micro_planners.semesters_id',$request->semesters_id);
            }
            if($request->months_id){
              $query->where('micro_planners.months_id',$request->months_id);
            }
            if($request->subjects_id){
              $query->where('micro_planners.subjects_id',$request->subjects_id);
            }
            if($request->teacher_id){
              $query->where('micro_planners.created_by',$request->teacher_id);
            }
           if ($request->publish_date_start != "" && $request->publish_date_end != ""){
                $query->whereBetween('micro_planners.publish_date', [$request->publish_date_start, $request->publish_date_end]);
    ...
    public function add(Request $request){
     ...
     public function store(Request $request){
        ...
        Micro_planner::create($request->all());
        //return redirect($this->base_route.'/head');
        return redirect('Lms/micro_planner')->with('message_success',' Added Successfully');
    }
      ...
      public function edit(Request $request,$id){
        ...
        $data['row']=Micro_planner::find($id);
        //dd($data['row']);
        ...
        if($request->all()){
            ...
            return redirect()->route('Lms.micro_planner')->with('message_success','Recoard Updated');
        }
        $data['Lesson_plans']=Micro_planner::select('micro_planners.id','micro_planners.months_id','micro_planners.faculty','micro_planners.semesters_id','subjects_id','chapter_no_id','faculties.faculty','semesters.semester','timetable_subjects.title as subject','chapter_no.title')
        ->leftjoin('faculties','micro_planners.faculty','=','faculties.id')
        ->leftjoin('semesters','micro_planners.semesters_id','=','semesters.id')
        ->leftjoin('timetable_subjects','micro_planners.subjects_id','=','timetable_subjects.id')
        ->leftjoin('chapter_no','micro_planners.chapter_no_id','=','chapter_no.id')
    ...
    public function delete(Request $request,$id){
       ...
       $data['row']=DB::table('micro_planners')->where('id',$id)->delete();
        if(!$data['row']){
